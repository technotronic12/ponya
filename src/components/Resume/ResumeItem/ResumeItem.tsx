...
export class ResumeItem extends React.Component<{ item: IResumeItemData, right: boolean }, {}> {
  ...
  render() {
    return (
      <div>
        <p className={ `${ style.years } ${ this.props.right ? style.right : style.left }` }>{ this.props.item.years }</p>
        ...
        { this.props.item.images && this.props.item.images.map((image, index) =>
          <img alt="nodejs js typescript scala developer" key={ index } className={ style.resumeIcon } src={ image }/>) }
      </div>
    );
  }
}
